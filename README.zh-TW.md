...
- **會累積的知識 wiki**：每張 mask 的 `knowledge/` 是一個小型常駐 wiki —— 目錄、時間序 `log.md`、頁與頁之間的 `[[交叉連結]]` —— 仿 Karpathy 的 [LLM-wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f) 概念。重新蒸餾是**疊加**而非重練，`mask coverage` 還會稽核它的完整性（孤兒頁、壞連結、查無出處的主張）。
