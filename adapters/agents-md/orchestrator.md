...
- The distillation flow, scope-resolution protocol, extraction discipline (`{{recipe}}`, five passes; skeletons in `{{templates}}`), and citation contract (tag facts [src:], say speculation if unsupported) are identical to the Claude Code version.
