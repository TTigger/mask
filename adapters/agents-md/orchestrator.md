...
- The distillation flow, scope-resolution protocol, extraction discipline (voice sources → `{{recipe}}`, code/repo sources → `{{code_recipe}}`, five passes; skeletons in `{{templates}}`), and citation contract (tag facts [src:], say speculation if unsupported) are identical to the Claude Code version.
